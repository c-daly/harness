...
Current implementation branch: `feat/checked-ensemble`, based on `main` at
`f997a21` after PR55 merged. Its mixed-runtime journey and cross-task export
ownership correction are on main, alongside activity inspection, timer grants,
durable admission counts and targeted operator cancellation.
...
## Shared usage stop limits and durable accounting

PR45 merged at `0184e00` after its configuration review fix (`a0d315d`). This
increment continues M5 with core-owned accounting across the root session,
native and external child responses, internal inference, and retry attempts.

**Implemented:** optional CLI input/output-token and estimated-cost stop limits;
preflight against the effective route's explicit accounting declaration and
finite token prices; a shared root event ledger; and read-only `/budget` and
`harness budget` inspection. Stops prevent further model admission after settled
usage crosses a configured limit. Existing per-response and concurrency bounds
remain in force. Already running calls can cross a limit and still retain their
outputs; these are not exact pre-call reservations or provider billing caps.

Failed, cancelled, malformed, and interrupted attempts retain reported usage
as lower bounds with explicit unknown completeness. They cannot be treated as
free retries or erased by a later successful call. Rates are frozen after
routing and before provider entry. Unknown required accounting holds further
bounded work. Cost uses the existing input/output-rate convention, with decimal
arithmetic; cache-specific charges/discounts and other billing items are outside
this estimate.

The root owns durable starts and settlements for its descendants. Restart restores
its usage and limits, repairs orphaned attempts as unknown, and permits only
narrower limits. A linked child must resume through its owning root. Legacy
untracked work stays unknown. Handoff cannot widen captured usage limits, and
portable continuation includes source usage provenance without granting budget
authority in another frontend. No plugin internals are imported or modified.

**Validation:** 38 new regressions cover actual TUI compositor inspection,
retained drafts and task obligations, readable CLI stops, routing, retries,
stream rejection, cancellation, parallel/nested children, root restart without
child logs, orphan recovery, ledger-write failure, export, and failed
external-to-native handoff accounting. Final checks also reproduce and fix export
reading a newer budget prefix than its task snapshot, and linked-child resume
tearing down the current TUI before refusal.

The full Python 3.13 checkpoint passed **2,005 tests, seven skipped**, with six
existing MCP deprecation warnings (467.40 s). Python 3.12 passed **329 affected
tests** (97.16 s). After the two final snapshot/resume fixes, **135 affected tests
passed on each Python version** (52.39 / 54.21 s), and all **10 existing TUI resume
tests** passed (32.48 s). The full suite was not repeated after those localized
fixes. All 215 core/test hashes remained unchanged during final validation.
Ruff, whitespace checks, offline sdist/wheel build, isolated wheel CLI help and
all 81 module imports passed; all 82 packaged core files match source. Skips
remain missing Anthropic/Ollama fixtures and the opt-in live Antigravity probe.
Scripted external transport checks are not live accounting conformance. Hosted
CI and review are pending publication.

**Remaining:** general runtime budget configuration, exact pre-call reservations
and live provider accounting qualification; broader edit ownership/isolated
worktrees, live mixed-agent and installed-plugin reconciliation; and isolated
source improvement validation/promotion/rollback. M4 held-out quality and the
explicit correction-to-repair loop remain open, as does M6 daily-use qualification.
No improved prompt, model, or source patch is automatically adopted here.


## Core execution controls

This increment returns to M5 runtime configuration on `feat/execution-controls`,
stacked above the still-open PR46 at `b0e540b`. The independent small-model
measurement branch is preserved and is not part of this change.

**Implemented:** `/execution` inspection and idle operator configuration, plus
matching CLI flags for task, native conversational request, coordination,
call-count, descendant, depth and active-capacity limits. Settings are recorded
before changes, restored on resume, inherited by children and `/clear`, and
restored from the destination on `/resume`. Partial CLI overrides retain the
other stored fields. Invalid stored settings refuse resume before its boundary
or the current TUI's teardown. Changes do not refund current counters.

Fresh tasks inherit the configured task timeout. Explicit/serialized task caps
and reconciled source handoff limits remain binding. Source narrowing is durable.
Native request limits apply after routing. Fallback retains the original native
request's remaining budget across replacements. Codex, Claude Code and
Antigravity default process timers follow the owned call through context-local
bindings; explicit adapter caps and setup/server bounds still apply. No plugin
implementation or live model configuration is changed.

**Stop semantics:** an expired task time budget remains an incomplete `deadline`
outcome with a readable error. An inner operation timeout before that budget
expires is recorded separately as `timeout`. Cancellation and cleanup ownership
remain unchanged. These are finite elapsed-time controls, not a claim that the
harness can detect a hang from silence or assess useful progress.

**Validation:** 49 new regressions cover both CLI paths, rendered TUI inspection,
draft preservation, core command precedence, session rebuilds, event validation,
failed configuration writes, explicit/replayed task caps, child inheritance,
source handoff, timeout causes and cleanup, and actual subprocess adapters with
scripted responses. The broader fallback checks caught and corrected timeout
refunding at replacement; the 84 focused execution/fallback checks pass.
The Python 3.13 full-suite checkpoint finished with **2,054 passed, seven
skipped and one failed** (427.13 s): the existing math-width failure described
below. Python 3.12 passed **477 affected tests, one skipped** (228.76 s). All 225
core/test files remained unchanged during that checkpoint. A final injected
resume-write failure exposed a live-lock leak; after fixing it and adding its
regression, **130 affected tests passed on each version** (26.77 / 26.00 s).
The full suite was not repeated after that localized cleanup fix. Ruff,
whitespace checks, offline sdist/wheel build, clean wheel CLI startup and all
82 module imports pass; all 83 packaged core files match source. Process tests
use scripted external executables, not live provider qualification.

**Remaining:** activity/progress-sensitive supervision, suspected-stall recovery,
active-budget extension, durable call/descendant consumption and exact usage
reservations; live mixed-agent accounting, edit/worktree ownership, installed
plugin reconciliation, isolated source-improvement validation/promotion/rollback,
and M4/M6 empirical and daily-use gates. Settings persistence does not make the
existing per-process call counters a durable spending ledger. The user guide is
[execution controls](../../execution-controls.md).

The broad suite also exposed `test_complex_inline_math_wraps_with_prose_in_document_order`
...
## Durable root admission counts

Model-attempt, tool-call and descendant reservations now survive ordinary resume.
The shared execution budget owns one root ledger: each reservation is fsynced
before execution, child logs record ownership links, and restoring root totals
does not depend on child logs. Retries consume separate model reservations;
denied tools retain their admitted reservations. Count-limit refusals do not
consume another slot. Cancellation and failure release live capacity without
refunding cumulative work. Historical capacity, activity and timers never resume
as live operations.

The fold validates count transitions and rejects malformed known records before
resume appends a run boundary. Failed or rewritten writes hold the whole live
tree. A reservation that reached disk before an ambiguous append failure stays
consumed after restart. A second independent budget cannot claim the same live
session. `/execution` displays durable consumption and any accounting hold; the
TUI checks destination ownership/integrity before teardown. Portable task JSON
carries counts and its source prefix as evidence only. Handoff retains captured
counts durably, without recharging descriptive start/proposal events; existing
child-effect reconciliation limits remain in place.

Older root-only sessions migrate through conservative recorded-start/retry/tool
counts, with a visible qualification. Legacy child spawns leave tree consumption
unknown and hold further admission; exporting and starting a new session remains
available. Older binaries cannot enforce the new event types. Work performed by
an older writer after a recorded resume is treated as a legacy interval on the
next upgrade, including the child-accounting hold when needed.

The broad Python 3.13 run passed **2,154 tests, seven skipped**, with six existing
MCP deprecation warnings and one fault-injection test failure (493.96 s). That
test made every session log unreadable while intending to simulate an unreadable
child terminal; root accounting restoration therefore encountered the injected
failure first. The fixture now keeps the root readable and fails only child
reads. After that test-only correction, **109 related tests pass on Python 3.13
(6.08 s) and Python 3.12 (6.67 s)**. The broader affected Python 3.12 set passed
**493 tests (274.13 s)**. The full suite was not repeated for the fixture change.

The 28 new regressions include actual process exit after reservation, restart
with missing child logs, retries, cancelled coordinators/children, failed and
ambiguous writes, rewriting, corrupt/nonmonotone records, legacy migration and
older-writer intervals, ownership, portable prefixes and final TUI composition.
All 231 source/test files were frozen during the broad runs; only the one
fault-injection fixture changed afterward. No application source changed.
Ruff, whitespace checks, offline wheel/sdist build, isolated installed CLI and
all 85 module imports pass. All 86 packaged core files match source.
These checks do not establish live provider accounting or daily-use qualification.

**Remaining:** exact pre-call token/cost reservations; independent operation timer
extension; useful-progress assessment and suspected-stall recovery; live mixed-
agent accounting and plugin reconciliation; edit/worktree ownership; isolated
source-improvement validation/promotion/rollback; and daily-use qualification.
M5 and M6 remain in progress.

## Operator cancellation of individual agent runs

Core `/activity cancel RUN_ID` now requests a stop of one live native or typed
external agent run, including its dependent work. The root-owned registry shares
execution ownership across the session tree and accepts a full ID or an
unambiguous prefix of at least eight characters. Inspection displays copyable
commands. This is an operator control, never a model tool or an automatic
silence/progress classifier. Individual activity, tool and coordinator IDs are
not separate cancellation targets.

The root fsyncs `agent_run_cancel_requested` before delivering cancellation.
Failed or rewritten requests send no cancellation; stale, foreign, ambiguous
and already stopping runs are refused. Stop intents are not terminal facts.
Each target records its own settled outcome, normally `cancelled` with reason
`operator_cancelled`; overlapping parent cancellation/deadlines retain their
normal outcomes. Restart restores no live controls or executable stop requests.
Portable JSON retains relevant source-prefix request metadata as evidence only.

Run bodies execute in owned tasks so a selected child can return a typed
cancelled result without cancelling its coordinator or independent siblings.
Sequential strategies and typed external runs retain the same boundary. Root
cancellation still unwinds its descendants. Capacity is released after cleanup;
consumed admissions are retained. Provider iteration and close operations settle
despite repeated caller cancellation or overlapping task/request timers. A
stopped subtree cannot admit fresh model, tool or child work through core after
a provider swallows cancellation, or turn the stop into a successful response.
Uncooperative runtime cleanup can still remain pending; this is cooperative
cancellation, not forcible termination or a guarantee about external effects.

The terminal remains usable during the stop, displays pending cleanup, preserves
draft input and queued prompts, and keeps delivered partial reply text visible.
Cancelling the root pauses follow-ups; task requirements remain unresolved until
ordinary evidence/acceptance checks resolve them. Neither cancellation nor
inspection extends budgets, and a stopping run cannot receive a timer grant.

The 20 new regression cases cover native/typed external boundaries, independent
and sequential children, root subtree cleanup, repeated cancellation and
coincident deadlines, started file writes, swallowed cancellation, write faults,
ID/authority refusal, restart/export, and actual 60/120-column TUI composition.
The existing coordinator capacity test now accepts either valid scheduling order
while still requiring all worker slots to fill and preserve shared limits.

Python 3.12 passes all **555 affected tests (308.92 s)**. The full Python 3.13
run reports **2,174 passed, seven skipped, six existing MCP deprecation warnings,
and one environment-dependent file-completion failure (570.14 s)**. A full
`/tmp` required workspace-backed scratch; Git then discovered the enclosing
repository and ignored the fixture files. The unchanged failing test passes
**in 2.38 s** with `GIT_CEILING_DIRECTORIES` set to the scratch directory. The
Python 3.12 affected run uses that corrected environment and includes this test.
An earlier Python 3.13 affected run passed 554 cases with the same environment
failure (302.87 s). The full suite was not repeated for the environment change.

All 234 source/test files remained frozen through these checks. Ruff and
whitespace checks pass. Offline wheel/sdist build, isolated installed CLI and
all 87 module imports pass; all 88 packaged core Python files match source.
These are automated/scripted results, not live mixed-provider or daily-use
qualification.
...
## Operator extensions of live coordinator timers

Core `/execution extend-coordinator COORDINATOR_ID ADDITIONAL_SECONDS` now
extends one live ensemble, panel, draft/refine or escalation timer, including
configured coordination agents and coordinators in child sessions. The existing
`/execution` inspection lists full IDs, remaining/total time and copyable grants.
A full ID or unambiguous prefix selects the intended coordination across queued
turns. Grants add finite positive time to the existing deadline and remain
independent of session defaults, enclosing tasks, member-agent/request/provider
timers, and sibling or nested coordinators.

A root-owned live registry checks operator/session/event-loop ownership, live
state and captured handoff restrictions. The root appends and fsyncs a
`coordination_budget_extended` intent before rescheduling. Failed or rewritten
writes leave the timer unchanged; a grant that reached disk before an ambiguous
failure remains only an intent. Expired, completed and stopping work cannot be
revived. A recorded parent-agent stop blocks a nested coordinator immediately,
before cancellation propagates to its worker. Cleanup retains ownership and
capacity until normal settlement; cumulative work counts are not refunded.

The target's settled report records the applied total timeout. Saved inspection
shows root grant intents, the initial deadline for an unconfirmed start, and the
settled report's total when available. Portable JSON carries relevant root-prefix
grants for owned coordinators and referenced child sessions as evidence only.
Missing aggregate terminals remain unconfirmed. Restart restores neither live
timers nor executable grants, and a child-only export does not retrieve the
root's grant records. No progress classifier or automatic extension is added.

Full Python 3.13 validation passes **2,208 tests, seven skipped, six existing
MCP deprecation warnings (602.67 s)**. All **468 affected tests pass on Python
3.12 (284.69 s)**. The 33 new cases cover real held members crossing the original
deadline, cumulative grants, expiry, nested and parallel isolation, authority/ID
refusals, failed and rewritten writes, cancellation races and cleanup, restart,
portable task selection, missing aggregate terminals, and 60/120-column terminal
composition. Existing strategy, handoff, task timer and cancellation regressions
are included; the focused set passed 176 tests (25.11 s).

All 236 source/test files remained frozen during those broad runs. Ruff,
whitespace checks, offline wheel/sdist build, isolated installed CLI and all
88 module imports pass. All 89 packaged core Python files match source. These
results establish automated behavior, not live mixed-provider or daily-use
qualification.

**Remaining:** exact pre-call token/cost reservations; provider and member-agent
timer extensions; useful-progress assessment and suspected-stall recovery;
live mixed-agent supervision/accounting and plugin reconciliation; edit/worktree
ownership; isolated source-improvement validation/promotion/rollback; and daily-
use qualification. M5 remains in progress and M6 remains pending.

### Inherited PR53 CI: expire the timer after mutation entry

PR53's Python 3.12 CI finished after merge with one further timing-sensitive
failure: a file-mutation lifetime test's 0.15-second task cap sometimes expired
before its file worker entered. Its cleanup assertion therefore never ran.
The test now starts with a bounded setup window and expires the real registered
root timer after confirmed worker entry. Cancellation, root/delegated ownership,
notice-write failure, retained file bytes, and terminal ordering assertions stay
in place; failure cleanup also cancels any remaining task.

This test-only correction followed the broad runs above. All **19 file-mutation
lifetime cases pass on Python 3.12 (2.79 s) and Python 3.13 (3.22 s)**; Ruff and
whitespace checks pass. No application source changed after the broad runs.
The full suite was not repeated for the fixture correction.

## Live mixed-runtime workflow and direct-operation export ownership

The new [opt-in journey](../../mixed-runtime-qualification.md) combines the
installed Qwen3-8B local inference model, the configured `openai/gpt-5` native
route and Codex CLI 0.154.0 through core task and coordination contracts. A fixed
operator driver supplies a public retry-budget task. The models read the fixture
and write separate files through Harness tools; the driver independently checks
the actual artifacts. Memory and agent-swarm remain plugins and are absent from
this narrow journey.

The live run exposed a real export omission: directly invoked coordinations
have no tool-call ID. Core now records their owning `agent_run_id` on starts and
terminals, and on child spawns. Task export includes direct operations using that
owner and exposes it as `run_id`. Unrelated concurrent work is excluded; changed
ownership, conflicting tool/run IDs and records outside their runs are refused.
Blocked direct outcomes and unconfirmed starts retain their distinct states.
Old tool-owned records remain exportable. Old direct records without an owning
call or run remain in the source log and are omitted rather than assigned by
temporal overlap.

The [final measurement](../../handoffs/2026-09-11-mixed-runtime/report.json)
passes **23 of 24 checks and deliberately reports `passed: false`**. The typed
external runtime was stopped during its first real Harness file-tool call,
paused at a controlled fixture gate. Target settlement took **0.178 seconds**.
Native siblings retained their results; restart restored task, context, limits
and accounting exactly; a fresh external attempt completed; and portable export
retained both coordinations and the cancellation. No live work or acceptance was
replayed. Only the unfinished external member was retried.

The failed check is substantive: local 8B included an exhausted job and reported
six remaining attempts instead of four. Its read returned the complete fixture.
The remote and resumed external artifacts were correct. All four sequential
development attempts produced the same incorrect local decision. The first
attempt also had two driver configuration/lifecycle errors; later attempts
exposed and fixed export ownership. Earlier runs are retained as development
observations, not hidden or counted as independent held-out trials. No prompt,
fixture, model capability or adoption policy was changed to force a passing
result. M3's earlier fact-copying result does not establish rule-classification
quality.

Final counts were nine model calls, nine tool calls and six descendants. One
cancelled usage attempt remained unknown across restart. The fresh Codex attempt
reported 64,401 input tokens for this small fixture, illustrating the external
runtime's additional context/accounting. Reported amounts are observations, not
complete usage or an exact cost reservation. The owned local server was stopped;
the shared host/GPU was not an isolated performance environment.

**Remaining:** task-specific acceptance and recovery for incorrect participant
results; broader live supervision/accounting and interruption phases; installed
memory/swarm workflow reconciliation; broader portable continuation and edit
ownership; source-improvement validation/promotion/rollback; and daily-use
qualification. M5 remains in progress and M6 remains pending. This increment
adds measured evidence and fixes its observed core export gap; it does not grant
the resident local classifier new authority.

Validation passes **2,237 tests on Python 3.13, seven skipped, six existing MCP
deprecation warnings (537.47 s)** and **280 affected tests on Python 3.12
(34.49 s)**. The 29 new cases cover the complete scripted journey, incorrect or
missing artifacts, missing/cyclic lineage, failed restart, explicit and legacy
operation ownership, concurrent unowned work, blocked admission, missing
terminals and corrupt ownership records. An earlier focused set passed 147
tests (30.32 s).

All 233 frozen core, top-level test and driver files remained unchanged through
the broad runs and final live measurement. Ruff, whitespace checks, offline
wheel/sdist build, isolated installed CLI and all 88 module imports pass; all
...
## Checked ensemble selection

PR55 merged at `f997a21`. The next increment follows the
[checked ensemble plan](2026-09-11-checked-ensemble.md): reuse the existing
escalation evidence checker for concurrent experts and optional synthesis.
The calling task's objective, requirements and source are frozen before fan-out.
Each expert and judge executes those requirements in its own recorded child
task; core rechecks its immutable output/tool evidence and lineage.

With declared requirements, only complete, untruncated, all-passing candidates
enter the vote or judge prompt. A wrong majority cannot defeat passing evidence.
A judge must independently pass the same checks. If none pass, no judge runs;
complete candidate texts remain available with unverified labels. Failed
synthesis retains its output as incomplete. Partial/interrupted siblings keep
the existing incomplete aggregate status even alongside a passing candidate.
Every participant, grade and disagreement stays in the report.

The tool, Python API and configured ensemble agents expose the strict
`require_checks` boolean. Existing criteria cannot be disabled by a model's
arguments; the flag additionally refuses missing criteria before starting
children. Unchecked legacy voting/synthesis remains available when there is no
calling-task requirement. Parent task checks and explicit acceptance remain
separate from child selection. Memory and agent-swarm remain plugins.

The mixed-runtime driver's declared human-review requirement now keeps its
fresh external retry's aggregate/root incomplete despite complete participant
execution. Its oracle requires the recorded review hold and continues to
independently require exact files, successful calls, lineage, restart accounting,
retained work and cancellation. Four new oracle mutations reject a fabricated
review pass, missing review evidence, unchecked selection or false aggregate
completion. The earlier live report is unchanged and remains tied to `48f4453`.

Initial regressions reproduced unchecked majority and judge behavior. Additional
coverage exercises wrong/corrupt evidence, out-of-order checks, strict frontmatter,
tool-result provenance, the actual calling task, partial work, mixed native and
external contracts, final terminal rendering, read-only export and replay.
Interrupted read-only verification stays unconfirmed; its delayed completion
cannot append a late result. The deadline test expires the registered coordinator
only after verification starts, independently of machine speed.

The first full runs exposed eight obsolete frontmatter assertions that still
restricted checks to escalation; both runs were stopped after that diagnosis.
The corrected matrix accepts explicit true/false for ensemble and escalation
and still refuses unsupported strategies. No application source changed after
the initial broad runs. Full reruns use fresh Python 3.12/3.13 environments from
the lockfile. Missing offline wheels/registry metadata required dependency
downloads; no model, GPU or paid provider calls were made for this increment.

M5 remains in progress. These exact recorded checks do not establish general
semantic correctness, qualify the failed local classifier, or replace the
workflow's independent artifact oracle. Richer artifact predicates, useful live
workflows with accepted task evidence, installed-plugin reconciliation, broader
continuation/edit ownership and isolated source-improvement evaluation,
promotion and rollback remain. M6 daily-use qualification remains pending.

**Validation:** the complete locked-environment suites pass **2,280 tests on
Python 3.13 (603.35 s)** and **2,280 on Python 3.12 (610.47 s)**, each with seven
skips and six existing MCP deprecation warnings. This adds 33 cases overall.
All 35 frontmatter cases passed after correcting their obsolete assertions;
the earlier affected set passed 174 tests (28.93 s). All 240 core, test and
driver hashes remained frozen through the final full runs. Ruff, whitespace
