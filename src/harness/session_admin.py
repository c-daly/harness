...
class IntegrityReport(BaseModel):
    ...
    @staticmethod
    def _digest_payload(session_id: SessionId, findings: Iterable[Finding]) -> str:
    ...
    @classmethod
    def build(cls, session_id: SessionId, findings: list[Finding]) -> "IntegrityReport":
        digest = cls._digest_payload(session_id, findings)
...
def verify_session(base: Path, session_id: SessionId, mode: Literal["quick", "full"]) -> IntegrityReport:
    ...
    # Blob checks
    present = _list_blob_paths(base, session_id)
    ...
    return IntegrityReport.build(session_id, findings)
