"""Paired message-prompt experiments with a fixed, non-model grader.
...
def _measurement(case, values):
    def score(value):
        if value is None:
            return None
        if value.reason in {"classified", "uncertain"}:
            return value.kind == case.expected
...
def _metrics(suite, rows):
    def summarize(cases, side):
        values = [rows[c.id].get(side) for c in cases]
        passed = [_measurement(c, rows[c.id]).model_dump()[f"{side}_passed"]
                  for c in cases] if side != "rules" else [rule_message_kind(c.text) == c.expected for c in cases]
        ...
        return {"samples": len(cases), "measured": sum(p is not None for p in passed),
                ...
                "abstentions": sum(v.kind == "uncertain" for v in values if v is not None)
                if side != "rules" else sum(rule_message_kind(c.text) == "uncertain" for c in cases),
...
async def run_evaluation(kernel, plan_id, *, incumbent, config: EvaluatorConfig) -> ExperimentResult:
    ...
    if candidate.target != "prompt" or incumbent.sha256 != plan.incumbent_version:
        raise ValueError("message evaluation requires the planned incumbent and a prompt candidate")
    ...
    try:
        async with asyncio.timeout(config.timeout_seconds):
            for index, case in enumerate(suite.cases):
                ...
                for side in order:
                    ...
                    rows[case.id][side] = await kernel.semantics.interpret(
                        case.text, model=config.model, limits=config.limits,
        ...
        report = {"schema_version": 1, "run_id": run_id, "completion": status,
            ...
            "metrics": _metrics(suite, rows), "cases": [{
                **case.model_dump(), "rules": rule_message_kind(case.text),
        ...
        result = ExperimentResult(id=str(uuid4()), plan_id=plan.id,
            ...
            observations=tuple(_measurement(c, rows[c.id]) for c in suite.cases),
