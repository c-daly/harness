"""Supervised improvement of a shadow message prompt, using core records.
...
from harness.events import EvaluationRunStarted, SemanticObserved
from harness.fold import fold
from harness.improvement import Candidate, EvaluationPlan, Evidence, PromptChange, verdict
...
def default_prompt(session, *, persist=True):
    data = DEFAULT_MESSAGE_PROMPT.model_dump_json().encode()
    return session.blobs.put(data) if persist else BlobRef(sha256=hashlib.sha256(data).hexdigest(), size=len(data))


def prompt_selection(session, provider, model, *, limits=None, persist_default=True):
    ...
    change = state.prompt_changes.get(state.active_prompts.get(model))
    if change is None:
        return default_prompt(session, persist=persist_default), "builtin", None
    load_prompt(session.blobs, change.prompt)
    if change.configuration is not None:
        ...
        if (config.model != model or evaluator_version(provider, config) != change.evaluator_version
            ...
            return default_prompt(session, persist=persist_default), "suspended: evaluation configuration changed", change
    return change.prompt, change.action, change


def repeated_failures(session, model, prompt):
    ...
    for env in reversed(read_session(session.base, session.id, repair=False)):
        if not isinstance(env.event, SemanticObserved):
            continue
...
class PromptImprovementService:
    ...
    def _current(self, model):
        prompt, status, change = prompt_selection(self.kernel.session, self.kernel.provider, model)
    ...
    def status(self, model):
        ...
        prompt, status, _ = prompt_selection(self.kernel.session, self.kernel.provider, model, persist_default=False)
        failures = repeated_failures(self.kernel.session, model, prompt)
        return _safe(f"Message prompt for {model}: {prompt.sha256[:12]} ({status}); shadow only.\n"
    ...
    async def propose(self, *, model: ModelId):
        self._idle()
        async with self._lock:
            kernel, session = self.kernel, self.kernel.session
            incumbent, _ = self._current(model)
            failures = repeated_failures(session, model, incumbent)
            if len(failures) < 2:
            ...
            prompt = load_prompt(session.blobs, incumbent)
            result = await kernel.loop.dispatcher.dispatch_inference(provider=provider, pinned=True,
                request=InferenceRequest(model=model, purpose="improvement:propose",
                    messages=(Message.system_text(
                        "Propose a revised message-classification prompt after repeated invalid outputs. "
                        ...
                        "The prompt value is an object with function='message_kind', version=1, and instructions "
                        "containing the revised classifier instructions. hypothesis and expected_benefit are each "
                        "strings of at most 1024 characters. Do not return just the prompt object or a classification. "
                        "Preserve all message kinds, explicit stop/pause distinctions, uncertainty, and the rule "
                        "that acknowledgements cannot accept tasks. Change only prompt instructions. "
            ...
            if result.model != model or kernel.provider is not provider or self._current(model)[0] != incumbent:
            ...
            if (not proposal.prompt.instructions.strip()
                    or proposal.prompt.instructions.strip() == prompt.instructions.strip()):
                raise ValueError("proposal must contain a changed, nonblank message prompt")
    ...
    async def evaluate(self, candidate_id, *, experiment: PromptExperiment):
        self._idle()
        experiment = PromptExperiment.model_validate(experiment.model_dump())
        async with self._lock:
            kernel, session = self.kernel, self.kernel.session
            config = experiment.configuration
            state = read_improvements(session.base, session.id)
            candidate = state.candidates[candidate_id]
            incumbent, _ = self._current(config.model)
            if candidate.target != "prompt" or candidate.incumbent_version != incumbent.sha256:
                raise ValueError("candidate does not replace the current message prompt")
            load_prompt(session.blobs, candidate.artifact)
    ...
    def adopt(self, result_id, *, model: ModelId):
        ...
        incumbent, previous = self._current(model)
        change = PromptChange(id=uuid4().hex, model=model, action="adopt", result_id=result.id,
    ...
    def rollback(self, *, model: ModelId):
        self._idle()
        session = self.kernel.session
        state = read_improvements(session.base, session.id)
        previous = state.prompt_changes.get(state.active_prompts.get(model))
        if previous is None:
            raise ValueError("no prompt change to roll back for this model")
        restored = state.prompt_changes.get(previous.previous_id)
        change = PromptChange(id=uuid4().hex, model=model, action="rollback",
