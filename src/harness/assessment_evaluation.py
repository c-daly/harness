...
class AssessmentExperiment(BaseModel):
...
def prepare_assessment_experiment(kernel, experiment: AssessmentExperiment):
    ...
    from harness.semantic_assessment import CONTEXT_PROMPT, PROGRESS_PROMPT
    ...
    builtin = CONTEXT_PROMPT if experiment.suite.function == "context_selection" else PROGRESS_PROMPT
    if (not experiment.candidate.instructions.strip()
            or experiment.candidate.instructions.strip() == builtin.instructions.strip()):
        raise ValueError("comparison requires changed, nonblank assessment instructions")
    incumbent = session.blobs.put(builtin.model_dump_json().encode())
