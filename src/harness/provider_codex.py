...
class CodexProvider:
    ...
    async def complete(
        ...
        *,
        ...
        # be the only path back to real files.
        ...
        try:
            ...
            # Kill-then-clean, deterministically: closing gen here (rather than
            # letting an abandoned async generator fall to GC finalization)
            # guarantees the subprocess is gone before the scratch dirs it is
            # living in, and the MCP server it was talking to, disappear out
            # from under it.
            if gen is not None:
                await gen.aclose()
